...
void mostrarListaUsuarios(eUsuario usuario[], int l)
{
    printf("\nUsuarios: \n");
    for(int i=0;i<l;i++)
    {
...
void mostrarUsuarioConSuSerie(eUsuario usuario[], int l, eSerie serie[], int r)
{
    for(int i=0;i<l;i++)
    {
        for(int j=0;j<r;j++)
        {
            ...
            {
                printf("%s %s %s\n", usuario[i].nombre, serie[j].nombre,serie[j].genero);
            }
